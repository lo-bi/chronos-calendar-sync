    ...
    try:
        ...
        for unique_id, event in current_dict.items():
            if unique_id not in previous_dict:
                # Skip past events - only notify about future events
                if event.start and event.start < now:
                    logger.debug(f"Skipping past event: {event.get_calendar_title()} at {event.start}")
                    continue
        ...
        for unique_id, prev_data in previous_dict.items():
            if unique_id not in current_dict:
                ...
                if isinstance(prev_data, dict):
                    ...
                    event_date_str = prev_data.get('event_date', None)
                    
                    # Skip past events - only notify about future deletions
                    if event_date_str:
                        try:
                            event_date = datetime.fromisoformat(event_date_str)
                            if event_date < now:
                                logger.debug(f"Skipping past deleted event: {title} at {time_str}")
                                continue
                        except:
                            pass
                else:
        ...
        for unique_id, event in current_dict.items():
            if unique_id in previous_dict:
                # Skip past events - only notify about future modifications
                if event.start and event.start < now:
                    logger.debug(f"Skipping past modified event: {event.get_calendar_title()}")
                    continue
        ...
        # Save current state for next comparison (including event dates for deletion checks)
        new_state = {}
        for unique_id, event in current_dict.items():
            new_state[unique_id] = {
                'title': event.get_calendar_title(),
                'time_str': format_event_time(event),
                'event_date': event.start.isoformat() if event.start else None
            }
